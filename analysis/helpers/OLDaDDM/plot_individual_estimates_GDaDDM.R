...
.dots_e_estimates = read_estimates(fitdir=fitdir, study="dots", model="GDaDDM", dataset="e")
.dots_e_estimates$study = "Dots"
.numeric_e_estimates = read_estimates(fitdir=fitdir, study="numeric", model="GDaDDM", dataset="e")
...
ylow = -1
yhigh = 2
theta_gradient <- expand.grid(x=seq(xlow,xhigh,(xhigh-xlow)/gradient_resolution), y=seq(ylow,yhigh,(yhigh-ylow)/gradient_resolution))
...
ggsave(file.path(figdir, "GDaDDM_IndividualEstimates.pdf"), plt.compare.param.e,

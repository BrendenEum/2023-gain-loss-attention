...
function fit_RNPaDDM(; study::String = "error", dataset::String = "error")

    ########
    # Prep
    ########

    println("RNPaDDM"); println(study); println(dataset);
    ...
    CSV.write(gainOutPath * "RNPaDDM_GainEst_" * dataset * ".csv", gainEstimates_RNP)
    CSV.write(gainOutPath * "RNPaDDM_GainNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)
    ...
    CSV.write(lossOutPath * "RNPaDDM_LossEst_" * dataset * ".csv", lossEstimates_RNP)
    CSV.write(lossOutPath * "RNPaDDM_LossNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)
end

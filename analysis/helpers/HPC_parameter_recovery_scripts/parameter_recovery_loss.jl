...
# Stimuli
data = ADDM.load_data_from_csv("/central/groups/rnl/beum/scripts/expdataLoss.csv", "/central/groups/rnl/beum/scripts/fixationsLoss.csv"; stimsOnly=true);
nTrials = 100;
...
Threads.@threads for m in 1:length(sim_list_loss)
    ...
    fn = "/central/groups/rnl/beum/scripts/custom_addm_grid_loss.csv";
    ...
    output_path = "/central/groups/rnl/beum/parameter_recovery/loss/model_$(m).txt";
    write(output_path, string(MyModel)); 
    output_path = "/central/groups/rnl/beum/parameter_recovery/loss/fit_$(m).csv";

...
# Stimuli
data = ADDM.load_data_from_csv("/central/groups/rnl/beum/scripts/expdataGain.csv", "/central/groups/rnl/beum/scripts/fixationsGain.csv"; stimsOnly=true);
nTrials = 100;
...
Threads.@threads for m in 1:length(sim_list_gain)
    ...
    fn = "/central/groups/rnl/beum/scripts/custom_addm_grid_gain_benchmark.csv";
    ...
    output_path = "/central/groups/rnl/beum/parameter_recovery/benchmark/model_$(m).txt";
    write(output_path, string(MyModel)); 
    output_path = "/central/groups/rnl/beum/parameter_recovery/benchmark/fit_$(m).csv";

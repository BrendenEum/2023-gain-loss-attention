...
for (i in 1:N){
  ...
  for (j in 1:trials){
    gain_trial = addm_simulate_trial(
      b=b_list[i], d=d_list[i], t=t_gain_list[i], s=s_list[i], e=0, ref=0,
  ...
  for (j in (trials+1):(2*trials)){
    loss_trial = addm_simulate_trial(
      b=b_list[i], d=d_list[i], t=t_loss_list[i], s=s_list[i], e=0, ref=0,
...
ggsave(
  file.path(figdir, "sim_UNBOUNDEDaDDM_ChoiceBiases_Net.pdf"), 
...
ggsave(
  file.path(figdir, "sim_UNBOUNDEDaDDM_ChoiceBiases_Last.pdf"), 
...
plt.firstfix = bias.firstfix.plt(simData, xlim=firstfix_x_scale) +
  ...
  theme(legend.position="none")
ggsave(
  file.path(figdir, "sim_UNBOUNDEDaDDM_ChoiceBiases_First.pdf"), 
...
sim_aDDM_rt_oV = brm(
  ...
  file = file.path(tempdir, "sim_aDDM_rt_oV")
)
summary(sim_aDDM_rt_oV)

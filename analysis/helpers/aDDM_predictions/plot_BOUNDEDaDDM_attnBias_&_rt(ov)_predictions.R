...
b_list = runif(N, min=-.15, max=.15) %>% round(digits=1)
...
prFirstLeft_list = runif(N, min=.5, max=.95) %>% round(digits=2)
...
ggsave(
  file.path(figdir, "sim_BOUNDEDaDDM_ChoiceBiases_Net.pdf"), 
...
ggsave(
  file.path(figdir, "sim_BOUNDEDaDDM_ChoiceBiases_Last.pdf"), 
...
plt.firstfix = bias.firstfix.plt(simData, xlim=firstfix_x_scale) +
  ...
  theme(legend.position="none")
ggsave(
  file.path(figdir, "sim_BOUNDEDaDDM_ChoiceBiases_First.pdf"), 

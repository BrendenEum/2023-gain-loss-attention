...
t_loss_list = runif(N, min=0, max=1) %>% round(digits=1)
#e_list = runif(N, min=.005, max=.040)
...
for (i in 1:N){
  ...
  for (j in 1:trials){
    gain_trial = addm_simulate_trial(
      b=b_list[i], d=d_list[i], t=t_gain_list[i], s=s_list[i], e=0, ref=1,
  ...
  for (j in (trials+1):(2*trials)){
    loss_trial = addm_simulate_trial(
      b=b_list[i], d=d_list[i], t=t_loss_list[i], s=s_list[i], e=0, ref=-6,
...
ggsave(
  file.path(figdir, "sim_RaDDM_ChoiceBiases_Net.pdf"), 
...
ggsave(
  file.path(figdir, "sim_RaDDM_ChoiceBiases_Last.pdf"), 
...
plt.firstfix = bias.firstfix.plt(simData, xlim=firstfix_x_scale) +
  ...
  theme(legend.position="none")
ggsave(
  file.path(figdir, "sim_RaDDM_ChoiceBiases_First.pdf"), 
...
# #############################################################################
# 
# pdata = simData[simData$condition=="Loss",] %>%
#   group_by(studyN, subject, absvDiff) %>%
#   mutate(rt_corr = rt - mean(rt)) %>%
#   ungroup() %>%
#   group_by(studyN, subject, oV) %>%
#   summarize(mean_y = mean(rt_corr)) %>%
#   ungroup() %>%
#   group_by(studyN, oV) %>%
#   summarize(
#     y = mean(mean_y),
...
sim_RaDDM_rt_oV = brm(
  ...
  file = file.path(tempdir, "sim_RaDDM_rt_oV")
)
summary(sim_RaDDM_rt_oV)

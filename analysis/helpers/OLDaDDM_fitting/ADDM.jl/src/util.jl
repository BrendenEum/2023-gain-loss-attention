...
"""
function process_simulations(SimulatedData)
  ...
  for (i, cur_trial) in enumerate(SimulatedData)
      ...
      cur_beh_df = DataFrame(:parcode => 1, :trial => i, :choice => cur_trial.choice, :rt => cur_trial.RT, :LProb => cur_trial.LProb, :LAmt => cur_trial.LAmt, :RProb => cur_trial.RProb, :RAmt => cur_trial.RAmt)

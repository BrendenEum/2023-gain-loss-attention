...
fixCross.lastfix.reg <- function(data, study="error", dataset="error") {
  ...
  results <- brm(
    choice | trials(n) ~ vDiff*condition*location*fixCrossLoc + (1+vDiff*condition*location*fixCrossLoc | subject),
    data=data,
    family = binomial(link="logit"),
    file = file.path(tempregdir, paste0(study, "_FixCross_ChoiceBiases_Last_", dataset)))

...
function fit_DNPaDDM_custom_resolution(addm::aDDM, data::Dict{String, Vector{aDDMTrial}}, dGrid::Vector{Any}, σGrid::Vector{Any}, θGrid::Vector{Any}, bGrid::Vector{Any}, kGrid::Vector{Any}, subjectCount::Number)
    """
    """
    ...
    @showprogress for subject in collect(keys(data))
        ...
        dEst, σEst, θEst, bEst, kEst, NLL_Indiv = DNPaDDM_grid_search(addm, data, dGrid[ind], σGrid[ind], θGrid[ind], bGrid[ind], kGrid[ind], subject)
...
function fit_DNPaDDM(; study::String = "error", dataset::String = "error")

    ########
    # Prep
    ########

    println("DNPaDDM"); println(study); println(dataset);
    ...
    oldGainEstimates, oldNLLs = fit_DNPaDDM_custom_resolution(addm, dataGain, dInitialGrid, σInitialGrid, θInitialGrid, bInitialGrid, kInitialGrid, subjectCount)
    oldNLL = sum(oldNLLs)
    ...
    while Δ > grid_search_terminate_threshold
        ...
        newGainEstimates, newNLLs = fit_DNPaDDM_custom_resolution(addm, dataGain, dGrid, σGrid, θGrid, bGrid, kGrid, subjectCount)
        newNLL = sum(newNLLs)
        Δ = (oldNLL-newNLL)/oldNLL
        if Δ < 0
            global gainEstimates_DNP = oldGainEstimates
            global NLL = oldNLLs
        else
            global gainEstimates_DNP = newGainEstimates
            global NLL = newNLLs
        end

        print("Iteration "); println(iteration); print("Old NLL "); println(oldNLL); print("New NLL "); println(newNLL); print("Percent Change "); println(Δ); println(gainEstimates_DNP); iteration += 1; oldGainEstimates = newGainEstimates; oldNLL = newNLL; 

    end

    gainOutPath = "../../outputs/temp/" * study * "_"
    CSV.write(gainOutPath * "DNPaDDM_GainEst_" * dataset * ".csv", gainEstimates_DNP)
    CSV.write(gainOutPath * "DNPaDDM_GainNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)
    ...
    oldLossEstimates, oldNLLs = fit_DNPaDDM_custom_resolution(addm, dataLoss, dInitialGrid, σInitialGrid, θInitialGrid, bInitialGrid, kInitialGrid, subjectCount)
    oldNLL = sum(oldNLLs)
    ...
    while Δ > grid_search_terminate_threshold
        ...
        newLossEstimates, newNLLs = fit_DNPaDDM_custom_resolution(addm, dataLoss, dGrid, σGrid, θGrid, bGrid, kGrid, subjectCount)
        newNLL = sum(newNLLs)
        Δ = (oldNLL-newNLL)/oldNLL
        if Δ < 0
            global lossEstimates_DNP = oldLossEstimates
            global NLL = oldNLLs
        else
            global lossEstimates_DNP = newLossEstimates
            global NLL = newNLLs
        end

        print("Iteration "); println(iteration); print("Old NLL "); println(oldNLL); print("New NLL "); println(newNLL); print("Percent Change "); println(Δ); println(lossEstimates_DNP); iteration += 1; oldLossEstimates = newLossEstimates; oldNLL = newNLL; 

    end

    lossOutPath = "../../outputs/temp/" * study * "_"
    CSV.write(lossOutPath * "DNPaDDM_LossEst_" * dataset * ".csv", lossEstimates_DNP)
    CSV.write(lossOutPath * "DNPaDDM_LossNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)
end

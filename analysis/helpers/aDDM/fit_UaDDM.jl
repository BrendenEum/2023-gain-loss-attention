...
function fit_UaDDM_custom_resolution(addm::aDDM, data::Dict{String, Vector{aDDMTrial}}, dGrid::Vector{Any}, σGrid::Vector{Any}, θGrid::Vector{Any}, bGrid::Vector{Any}, subjectCount::Number)
    """
    """
...
function fit_UaDDM(; study::String = "error", dataset::String = "error")

    ########
    # Prep
    ########

    println("UaDDM"); println(study); println(dataset);
    ...
    oldGainEstimates, oldNLLs = fit_UaDDM_custom_resolution(addm, dataGain, dInitialGrid, σInitialGrid, θInitialGrid, bInitialGrid, subjectCount)
    oldNLL = sum(oldNLLs)
    ...
    while Δ > grid_search_terminate_threshold
        ...
        newGainEstimates, newNLLs = fit_UaDDM_custom_resolution(addm, dataGain, dGrid, σGrid, θGrid, bGrid, subjectCount)
        newNLL = sum(newNLLs)
    ...
    CSV.write(gainOutPath * "UaDDM_GainEst_" * dataset * ".csv", gainEstimates)
    CSV.write(gainOutPath * "UaDDM_GainNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)
    ...
    oldLossEstimates, oldNLLs = fit_UaDDM_custom_resolution(addm, dataLoss, dInitialGrid, σInitialGrid, θInitialGrid, bInitialGrid, subjectCount)
    oldNLL = sum(oldNLLs)
    ...
    while Δ > grid_search_terminate_threshold
        ...
        newLossEstimates, newNLLs = fit_UaDDM_custom_resolution(addm, dataLoss, dGrid, σGrid, θGrid, bGrid, subjectCount)
        newNLL = sum(newNLLs)
        Δ = (oldNLL-newNLL)/oldNLL
        if Δ < 0
    ...
    CSV.write(lossOutPath * "UaDDM_LossEst_" * dataset * ".csv", lossEstimates)
    CSV.write(lossOutPath * "UaDDM_LossNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)

...
p.exampleaDDM <- ggplot(data = pdata.exampleaDDM, aes(x=time, y=evid)) +
  ...
  annotate(geom='text', x=70, y=1.09, label='Choice = Left', size=9/.pt) +
  geom_hline(yintercept=-1, color='black', linetype='dashed') +
  annotate(geom='text', x=90, y=-1.09, label='Choice = Right', size=9/.pt) +
  ylab("Relative Decision Variable") +
  xlab("Time (ms)") +
  
  annotate(geom='text', x=1118 + 105, y= .20 - .28, label=TeX('$v_{left} = 1$'),  size=9/.pt) +
  annotate(geom='text', x=1138 + 105, y= .03 - .28, label=TeX('$v_{right} = 1$'), size=9/.pt) +
  annotate(geom='text', x=1130 + 134, y=-.14 - .28, label=TeX('$d = .003$'),      size=9/.pt) +
  annotate(geom='text', x=1110 + 136, y=-.29 - .28, label=TeX('$\\sigma = .02$'), size=9/.pt) +
  annotate(geom='text', x=1110 + 138, y=-.44 - .28, label=TeX('$\\theta = .52$'), size=9/.pt) +
  annotate(geom='text', x=1110 + 138, y=-.59 - .28, label=TeX('$\\b = .02$'),     size=9/.pt)

plot(p.exampleaDDM)
ggsave(
  file.path(figdir, "aDDM_example_gain.pdf"),

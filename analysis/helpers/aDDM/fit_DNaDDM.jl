...
function fit_DNaDDM(; study::String = "error", dataset::String = "error")

    ########
    # Prep
    ########
    
    println("DNaDDM"); println(study); println(dataset);
    ...
    CSV.write(gainOutPath * "DNaDDM_GainEst_" * dataset * ".csv", gainEstimates_DN)
    CSV.write(gainOutPath * "DNaDDM_GainNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)
    ...
    CSV.write(lossOutPath * "DNaDDM_LossEst_" * dataset * ".csv", lossEstimates_DN)
    CSV.write(lossOutPath * "DNaDDM_LossNLL_" * dataset * ".csv", Tables.table(NLL), writeheader=false)
end

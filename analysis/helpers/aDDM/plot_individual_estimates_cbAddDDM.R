...
.dots_e_estimates = read_estimates(fitdir=fitdir, study="dots", model="cbAddDDM", dataset="e")
.dots_e_estimates$study = "Dots"
.numeric_e_estimates = read_estimates(fitdir=fitdir, study="numeric", model="cbAddDDM", dataset="e")
...
coord.lim <- 5
buffer = coord.lim/20
theta_gradient <- expand.grid(x=seq(-buffer,coord.lim,coord.lim/gradient_resolution), y=seq(-buffer,coord.lim,coord.lim/gradient_resolution))
...
plt.compare.k.e <- ggplot(data=pdata) +
  ...
  scale_y_continuous(breaks = c(0, coord.lim/2, coord.lim), labels=c("0", "2.5", "5.0"), limits = c(-buffer,coord.lim)) +
  scale_x_continuous(breaks = c(0, coord.lim/2, coord.lim), labels=c("0", "2.5", "5.0"), limits = c(-buffer,coord.lim)) 
...
plt.compare.c.e <- ggplot(data=pdata) +
  ...
  scale_y_continuous(breaks = c(0, .001), labels=c("0", ".001"), limits = c(-buffer,coord.lim)) +
  scale_x_continuous(breaks = c(0, .001), labels=c("0", ".001"), limits = c(-buffer,coord.lim))

plt.compare.param.e <- grid.arrange(
  plt.compare.d.e, plt.compare.s.e, plt.compare.b.e, plt.compare.k.e, plt.compare.c.e, 
  ncol=2
)

ggsave(
  file.path(figdir, "cbAddDDM_IndividualEstimates.pdf"), 
  plt.compare.param.e,

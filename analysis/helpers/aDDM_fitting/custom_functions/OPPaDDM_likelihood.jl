...
function OPPaDDM_likelihood(;model::ADDM.aDDM, trial::ADDM.Trial, timeStep::Number = 10.0, stateStep::Number = 0.01)
    ...
    ZeroAmt_RD = trial.maxOutcome - 0

    LAmt_RD = trial.maxOutcome - trial.LAmt
    vL = (trial.LProb * LAmt_RD) + ((1-trial.LProb) * ZeroAmt_RD)

    RAmt_RD = trial.maxOutcome - trial.RAmt
    vR = (trial.RProb * RAmt_RD) + ((1-trial.RProb) * ZeroAmt_RD)
    ...
    # choice.
    if min(trial.valueLeft, trial.valueRight) >= 0 # in gains
        likelihood = 0
        if trial.choice == -1 # Choice was left (L=-1).  
            if probUpCrossing[end] > 0
                likelihood = probUpCrossing[end]
            end
        elseif trial.choice == 1 # Choice was right (R=1). 
        ...
        if trial.choice == -1 # Choice was left (L=-1). 
            if probDownCrossing[end] > 0
                likelihood = probDownCrossing[end] # Reverse this because we're testing if they choose the opposite of worst!
            end
        elseif trial.choice == 1 # Choice was right (R=1). 
            if probUpCrossing[end] > 0 
                likelihood = probUpCrossing[end] # Reverse.
            end
        end
    end

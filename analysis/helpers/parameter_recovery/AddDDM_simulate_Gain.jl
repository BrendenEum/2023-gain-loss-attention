...
predir = "results_AddDDM_Gain/";

# Custom functions
include("custom_simulators/AddDDM_simulate_trial.jl")
...
sim_grid = collect(product(d_grid, σ_grid, η_grid));
sim_grid_df = DataFrame(sim_grid, ["d", "sigma", "eta"]);
...
for i in 1:nrow(sim_grid_df)
    ...
    MyModel = ADDM.define_model(d = row.d, σ = row.sigma, θ = 1.0, η = row.eta, bias = 0.0, nonDecisionTime = 100, decay = 0.0);

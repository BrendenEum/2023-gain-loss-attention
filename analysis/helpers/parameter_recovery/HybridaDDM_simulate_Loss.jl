...
predir = "results_HybridaDDM_Loss/";

# Custom functions
include("custom_simulators/HybridaDDM_simulate_trial.jl")
simulator_fn = HybridaDDM_simulate_trial;

# Stimuli for loss simulations
expdata = "data_for_sims/expdataLoss_train.csv";
fixdata = "data_for_sims/fixationsLoss_train.csv";
...
sim_grid = collect(product(d_grid, σ_grid, θ_grid, η_grid));
sim_grid_df = DataFrame(sim_grid, ["d", "sigma", "theta", "eta"]);
...
for subject in 1:nrow(sim_grid_df)
    ...
    MyModel = ADDM.define_model(d = row.d, σ = row.sigma, θ = row.theta, η = row.eta, bias = 0.0, nonDecisionTime = 100, decay = 0.0);
    ...
    for (i, cur_trial) in enumerate(SimData)
        ...
        cur_beh_df = DataFrame(
            :parcode => subject, :trial => i, :condition => "Loss", :choice => cur_trial.choice, :rt => cur_trial.RT, 

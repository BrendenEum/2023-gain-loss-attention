...
.dataset = "e"
.nTrials = "146_trials"
.fn = "HybridaDDM_IndividualEstimates_E.pdf"
.fn_csv = "HybridaDDM_IndividualEstimates_E.csv"
...
# Limit to just RaDDM
.data = .data[.data$likelihood_fn=="HybridaDDM",]
...
save_est = data[,c("study", "subject", "condition", "d", "sigma", "theta", "eta")] %>% na.omit()
...
pdata_raw = pivot_wider(
  ...
  values_from = c("d","sigma","theta","eta")
)
...
plt1.compare.d.e <- ggplot(data=pdata) +
  ...
  coord_cartesian(xlim = c(-.001, .017), ylim = c(-.001, .017), expand=T) +
  scale_y_continuous(breaks = c(0, .008, .016), labels=c("0", ".008", ".016")) +
  scale_x_continuous(breaks = c(0, .008, .016), labels=c("0", ".008", ".016")) +
...
plt1.compare.e.e <- ggplot(data=pdata) +
  geom_abline(intercept=0, slope=1, color=exact) +
...
combo_plot = (plt1.compare.d.e + plt1.compare.s.e + plt1.compare.t.e + plt1.compare.e.e) + 
  plot_layout(guides = 'collect', nrow = 1)

ggsave(file.path(.figdir, .fn), combo_plot, height=3.5, width=11, units="in")

...
load(file.path(tempdir, "fdots_sim_cbAddDDM.RData"))
load(file.path(tempdir, "fnumeric_sim_cbAddDDM.RData"))
...
source("Sim_Choice.R")
sim.choice = sim.choice.plt(cfr_all, xlim=c(-1,1), legendposition=c(.15, .85))
ggsave(
  file.path(figdir, paste0("sims_choice_cbAddDDM", ext)), 
...
ggsave(
  file.path(figdir, paste0("sims_rt_cbAddDDM", ext)), 
...
ggsave(
  file.path(figdir, paste0("sims_net_cbAddDDM", ext)), 
...
ggsave(
  file.path(figdir, paste0("sims_last_cbAddDDM", ext)), 

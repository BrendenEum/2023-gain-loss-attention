...
date_folder = "2024.03.18.12.16" # yyyy.mm.dd.H.M of the results you want to look at.
...
figdir = datadir
optdir = file.path("../helpers/plot_options/")
...
Don't use the function for this one since we want this plot to have a legend for the colors. Plot some hidden Loss condition stuff.
```{r}
...
for (post in list(aDDM_post, AddDDM_post, RaDDM_post)) {
  ...
  lambda_p = post %>% mutate(variable="lambda", value=lambda) %>% calculateMarginal()
  ...
  lambda_pdata = rbind(lambda_pdata, lambda_p)
}

pdata = do.call("rbind", list(d_pdata, sigma_pdata, theta_pdata, eta_pdata, bias_pdata, lambda_pdata))
pdata$variable = factor(
  pdata$variable,
  levels = c("d","sigma","theta","eta","bias","lambda"),
  labels = c("d","sigma","theta","eta","bias","lambda")
)
...
for (row in 1:nrow(pdata)) {
  ...
  if (tInd==1) {theta = parse_number(x$V25); eta = parse_number(x$V28)} 
  else if (tInd==0) {theta = parse_number(x$V28); eta = parse_number(x$V31)}
  bias = parse_number(x$V13)
  lambda = parse_number(x$V16)
  ...
  else if (variable == "lambda") {pdata$truth[row]=lambda}
}

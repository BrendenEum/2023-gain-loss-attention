...
source(file.path(code_path,'cfr_to_addmdata.R'))
...
#  WHERE SHOULD DATA BE PULLED FROM? Convert it to data for addm fitting.
load(file.path(input_path, opt$data))

cur_sub = as.numeric(opt$subnum)

#data = data[(data$subject==cur_sub) & (data$Condition==opt$condition), ]
cfr = cfr %>%
  filter(subject==cur_sub & Condition==opt$condition)

data = cfr_to_addmdata(cfr)
...
grid_addm = read.csv(file.path(code_path, opt$grid))

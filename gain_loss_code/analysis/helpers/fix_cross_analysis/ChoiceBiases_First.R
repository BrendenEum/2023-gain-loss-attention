fixCross.firstfix.plt <- function(data, xlim) {
  ...
  pdata <- data[data$firstFix==T,] %>%
    group_by(studyN, subject, condition, location, fixCrossLoc, nfirstOtherVDiff) %>%
    summarize(
  ...
  pdata = pdata %>%
    group_by(studyN, subject, condition, fixCrossLoc, nfirstOtherVDiff) %>%
    summarize(
      choice.mean = mean(choice.mean)
    ) %>%
    ungroup() %>%
    group_by(studyN, condition, fixCrossLoc, nfirstOtherVDiff) %>%
    summarize(
      y = mean(choice.mean),
  ...
  plt <- ggplot(data=pdata, aes(x=nfirstOtherVDiff, y=y, color=condition)) +
    ...
    xlim(c(xlim[1],xlim[2])) +
    ylim(c(0,1)) +
    labs(y="Pr(Choose First Fix. Option)", x="Norm. First - Other E[V]") +
    theme_bw() +
    theme(
